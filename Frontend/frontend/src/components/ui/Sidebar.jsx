...
import { LayoutDashboard, Network, FileText, Box as BoxIcon, Server, ShoppingCart, Database, FileCheck, Coins, User, LogOut, Activity, Shield } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';

const menuItems = [
  { path: '/', icon: LayoutDashboard, label: 'Panou Principal' },
  { path: '/monitoring', icon: Activity, label: 'Monitoring', badge: 'ADMIN', adminOnly: true },
      ...
      <nav className="flex-1 p-4 space-y-2">
        {visibleMenuItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 relative ${isActive
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {item.badge && (
                  <span className={`ml-auto px-2 py-0.5 text-white text-xs rounded-full font-bold ${item.adminOnly ? 'bg-purple-500' : 'bg-green-500'
                    }`}>
                    {item.badge}
                  </span>
                )}
...
// Componenta separată pentru info utilizator
const UserInfo = () => {
    ...
    <div className="px-4 py-3 bg-dark-800 rounded-xl">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          {isAdmin && (
            <span className="px-1.5 py-0.5 bg-purple-500/20 text-purple-400 text-xs rounded font-semibold flex items-center gap-1">
